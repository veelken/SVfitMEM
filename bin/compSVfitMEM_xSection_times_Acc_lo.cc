...
/**
   \class compSVfitMEM_xSection_times_Acc_lo compSVfitMEM_xSection_times_Acc_lo.cc "TauAnalysis/SVfitStandalone/bin/compSVfitMEM_xSection_times_Acc_lo.cc"
...
#include "TauAnalysis/SVfitMEM/interface/HttXsectionWithTauDecays_lo.h"
...
//--- parse command-line arguments
  ...
  std::cout << "<compSVfitMEM_xSection_times_Acc_lo>:" << std::endl;

//--- keep track of time it takes the macro to execute
  TBenchmark clock;
  clock.Start("compSVfitMEM_xSection_times_Acc_lo");

//--- read python configuration parameters
  if ( !edm::readPSetsFrom(argv[1])->existsAs<edm::ParameterSet>("process") ) 
    throw cms::Exception("compSVfitMEM_xSection_times_Acc_lo") 
      << "No ParameterSet 'process' found in configuration file = " << argv[1] << " !!\n";

  edm::ParameterSet cfg = edm::readPSetsFrom(argv[1])->getParameter<edm::ParameterSet>("process");

  edm::ParameterSet cfgCompSVfitMEM_xSection_times_Acc_lo = cfg.getParameter<edm::ParameterSet>("compSVfitMEM_xSection_times_Acc_lo");
  
  double sqrtS = cfgCompSVfitMEM_xSection_times_Acc_lo.getParameter<double>("sqrtS");

  int mode = HttXsectionIntegrandWithTauDecays_lo::kLiterature;
  
  std::string leg1decayMode_string = cfgCompSVfitMEM_xSection_times_Acc_lo.getParameter<std::string>("leg1decayMode");
  int leg1decayMode = -1;
  double leg1Mass = 0.;
  ...
  } else throw cms::Exception("compSVfitMEM_xSection_times_Acc_lo") 
    << "Invalid Configuration parameter 'leg1decayMode' = " << leg1decayMode_string << " !!\n";
  double minLeg1Pt_1 = cfgCompSVfitMEM_xSection_times_Acc_lo.getParameter<double>("minLeg1Pt_1");
  double minLeg1Pt_2 = cfgCompSVfitMEM_xSection_times_Acc_lo.getParameter<double>("minLeg1Pt_2");
  double maxLeg1AbsEta = cfgCompSVfitMEM_xSection_times_Acc_lo.getParameter<double>("maxLeg1AbsEta");

  std::string leg2decayMode_string = cfgCompSVfitMEM_xSection_times_Acc_lo.getParameter<std::string>("leg2decayMode");
  int leg2decayMode = -1;
  double leg2Mass = 0.;
  ...
  double minLeg2Pt_1 = cfgCompSVfitMEM_xSection_times_Acc_lo.getParameter<double>("minLeg2Pt_1");
  double minLeg2Pt_2 = cfgCompSVfitMEM_xSection_times_Acc_lo.getParameter<double>("minLeg2Pt_2");
  double maxLeg2AbsEta = cfgCompSVfitMEM_xSection_times_Acc_lo.getParameter<double>("maxLeg2AbsEta");

  // define Higgs mass points 
  std::vector<double> mH = cfgCompSVfitMEM_xSection_times_Acc_lo.getParameter<vdouble>("mH");
  if ( !(mH.size() >= 1) ) throw cms::Exception("compSVfitMEM_xSection_times_Ac_loc") 
  ...
  unsigned maxObjFunctionCalls = cfgCompSVfitMEM_xSection_times_Acc_lo.getParameter<unsigned>("maxObjFunctionCalls");

  std::string intMode_string = cfgCompSVfitMEM_xSection_times_Acc_lo.getParameter<std::string>("intMode");
  int intMode = 1;
  if      ( intMode_string == "vamp"  ) intMode = SVfitMEM_lo::kVAMP;
  else if ( intMode_string == "vegas" ) intMode = SVfitMEM_lo::kVEGAS;
  else throw cms::Exception("compSVfitMEM_xSection_times_Acc_lo") 
    << "Invalid Configuration parameter 'intMode' = " << intMode_string << " !!\n";

  std::string outputFileName = cfgCompSVfitMEM_xSection_times_Acc_lo.getParameter<std::string>("outputFileName");
  std::cout << " outputFileName = " << outputFileName << std::endl;

  int verbosity = cfgCompSVfitMEM_xSection_times_Acc_lo.getParameter<int>("verbosity");
  ...
  for ( std::vector<double>::const_iterator mH_i = mH.begin();
    ...
    HttXsectionWithTauDecays_lo HttXsection_woAcc(sqrtS, *mH_i, pdfName.data(), mode, "", verbosity);
    ...
    HttXsectionWithTauDecays_lo HttXsection_wAcc(sqrtS, *mH_i, pdfName.data(), mode, "", verbosity);
