...
/**
   \class testHttXsectionWithTauDecays_lo testHttXsectionWithTauDecays_lo.cc "TauAnalysis/SVfitStandalone/bin/testHttXsectionWithTauDecays_lo.cc"
...
int main(int argc, char* argv[]) 
{
  ...
  //int mode = HttXsectionIntegrandWithTauDecays_lo::kMadgraph;
  ...
  intModes.push_back(HttXsectionWithTauDecays_lo::kVEGAS);
  intModes.push_back(HttXsectionWithTauDecays_lo::kVAMP);
  ...
  for ( std::vector<double>::const_iterator mH_i = mH.begin();
	...
	HttXsectionWithTauDecays_lo HttXsection_woAcc(sqrtS, *mH_i, pdfName.data(), mode, findFile(madgraphFileNames[*mH_i]), verbosity);
	...
	HttXsectionWithTauDecays_lo HttXsection_wAcc(sqrtS, *mH_i, pdfName.data(), mode, findFile(madgraphFileNames[*mH_i]), verbosity);
  ...
  TFile* outputFile = new TFile("testHttXsectionWithTauDecays_lo.root", "RECREATE");
  for ( std::vector<unsigned>::const_iterator numCalls_i = numCalls.begin();
	numCalls_i != numCalls.end(); ++numCalls_i ) {
    for ( std::vector<int>::const_iterator intMode = intModes.begin();
	  intMode != intModes.end(); ++intMode ) {
