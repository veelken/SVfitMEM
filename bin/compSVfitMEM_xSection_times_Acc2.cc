...
/**
   \class compSVfitMEM_xSection_times_Acc2 compSVfitMEM_xSection_times_Acc2.cc "TauAnalysis/SVfitStandalone/bin/compSVfitMEM_xSection_times_Acc2.cc"
...
//--- parse command-line arguments
  ...
  std::cout << "<compSVfitMEM_xSection_times_Acc2>:" << std::endl;

//--- keep track of time it takes the macro to execute
  TBenchmark clock;
  clock.Start("compSVfitMEM_xSection_times_Acc2");

//--- read python configuration parameters
  if ( !edm::readPSetsFrom(argv[1])->existsAs<edm::ParameterSet>("process") ) 
    throw cms::Exception("compSVfitMEM_xSection_times_Acc2") 
      << "No ParameterSet 'process' found in configuration file = " << argv[1] << " !!\n";

  edm::ParameterSet cfg = edm::readPSetsFrom(argv[1])->getParameter<edm::ParameterSet>("process");

  edm::ParameterSet cfgCompSVfitMEM_xSection_times_Acc = cfg.getParameter<edm::ParameterSet>("compSVfitMEM_xSection_times_Acc2");
  
  double sqrtS = cfgCompSVfitMEM_xSection_times_Acc.getParameter<double>("sqrtS");

  int mode = HttXsectionIntegrandWithTauDecays2::kLiterature;
  ...
  // apply/do not apply constraint that Higgs has zero transverse momentum
  bool applyZeroPtConstraint = cfgCompSVfitMEM_xSection_times_Acc.getParameter<bool>("applyZeroPtConstraint");
  double max_v_r = cfgCompSVfitMEM_xSection_times_Acc.getParameter<double>("max_v_r");
  if ( max_v_r < 0. ) max_v_r = sqrtS;
  ...
  for ( std::vector<double>::const_iterator mH_i = mH.begin();
    ...
    HttXsectionWithTauDecays2 HttXsection_woAcc(sqrtS, *mH_i, pdfName.data(), mode, "", verbosity);
    HttXsection_woAcc.setBR(branchingRatio);
    HttXsection_woAcc.disableAcceptanceCuts();
    if ( applyZeroPtConstraint ) {
      HttXsection_woAcc.enableZeroPtConstraint();
    } else {
      HttXsection_woAcc.disableZeroPtConstraint();
      HttXsection_woAcc.setMax_v_r_(max_v_r);
    }
    ...
    HttXsectionWithTauDecays2 HttXsection_wAcc(sqrtS, *mH_i, pdfName.data(), mode, "", verbosity);
    ...
    if ( applyZeroPtConstraint ) {
      HttXsection_wAcc.enableZeroPtConstraint();
    } else {
      HttXsection_wAcc.disableZeroPtConstraint();
      HttXsection_woAcc.setMax_v_r_(max_v_r);
    }
