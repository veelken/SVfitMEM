...
/**
   \class compSVfitMEM_xSection_times_Acc_nlo compSVfitMEM_xSection_times_Acc_nlo.cc "TauAnalysis/SVfitStandalone/bin/compSVfitMEM_xSection_times_Acc_nlo.cc"
   \brief Compute leading order gg -> Higgs -> tautau+jet cross-section with and without acceptance cuts
*/
...
//--- parse command-line arguments
  ...
  std::cout << "<compSVfitMEM_xSection_times_Acc_nlo>:" << std::endl;

//--- keep track of time it takes the macro to execute
  TBenchmark clock;
  clock.Start("compSVfitMEM_xSection_times_Acc_nlo");

//--- read python configuration parameters
  if ( !edm::readPSetsFrom(argv[1])->existsAs<edm::ParameterSet>("process") ) 
    throw cms::Exception("compSVfitMEM_xSection_times_Acc_nlo") 
      << "No ParameterSet 'process' found in configuration file = " << argv[1] << " !!\n";

  edm::ParameterSet cfg = edm::readPSetsFrom(argv[1])->getParameter<edm::ParameterSet>("process");

  edm::ParameterSet cfgCompSVfitMEM_xSection_times_Acc_nlo = cfg.getParameter<edm::ParameterSet>("compSVfitMEM_xSection_times_Acc_nlo");
  
  double sqrtS = cfgCompSVfitMEM_xSection_times_Acc_nlo.getParameter<double>("sqrtS");
  
  std::string leg1decayMode_string = cfgCompSVfitMEM_xSection_times_Acc_nlo.getParameter<std::string>("leg1decayMode");
  int leg1decayMode = -1;
  double leg1Mass = 0.;
  ...
  } else throw cms::Exception("compSVfitMEM_xSection_times_Acc_nlo") 
    << "Invalid Configuration parameter 'leg1decayMode' = " << leg1decayMode_string << " !!\n";
  double minLeg1Pt_1 = cfgCompSVfitMEM_xSection_times_Acc_nlo.getParameter<double>("minLeg1Pt_1");
  double minLeg1Pt_2 = cfgCompSVfitMEM_xSection_times_Acc_nlo.getParameter<double>("minLeg1Pt_2");
  double maxLeg1AbsEta = cfgCompSVfitMEM_xSection_times_Acc_nlo.getParameter<double>("maxLeg1AbsEta");

  std::string leg2decayMode_string = cfgCompSVfitMEM_xSection_times_Acc_nlo.getParameter<std::string>("leg2decayMode");
  int leg2decayMode = -1;
  double leg2Mass = 0.;
  ...
  double minLeg2Pt_1 = cfgCompSVfitMEM_xSection_times_Acc_nlo.getParameter<double>("minLeg2Pt_1");
  double minLeg2Pt_2 = cfgCompSVfitMEM_xSection_times_Acc_nlo.getParameter<double>("minLeg2Pt_2");
  double maxLeg2AbsEta = cfgCompSVfitMEM_xSection_times_Acc_nlo.getParameter<double>("maxLeg2AbsEta");

  // define Higgs mass points 
  std::vector<double> mH = cfgCompSVfitMEM_xSection_times_Acc_nlo.getParameter<vdouble>("mH");
  if ( !(mH.size() >= 1) ) throw cms::Exception("compSVfitMEM_xSection_times_Acc_nlo") 
  ...
  unsigned maxObjFunctionCalls = cfgCompSVfitMEM_xSection_times_Acc_nlo.getParameter<unsigned>("maxObjFunctionCalls");

  std::string intMode_string = cfgCompSVfitMEM_xSection_times_Acc_nlo.getParameter<std::string>("intMode");
  int intMode = 1;
  if      ( intMode_string == "vamp"  ) intMode = SVfitMEM_nlo::kVAMP;
  else if ( intMode_string == "vegas" ) intMode = SVfitMEM_nlo::kVEGAS;
  else throw cms::Exception("compSVfitMEM_xSection_times_Acc") 
    << "Invalid Configuration parameter 'intMode' = " << intMode_string << " !!\n";

  std::string outputFileName = cfgCompSVfitMEM_xSection_times_Acc_nlo.getParameter<std::string>("outputFileName");
  std::cout << " outputFileName = " << outputFileName << std::endl;

  int verbosity = cfgCompSVfitMEM_xSection_times_Acc_nlo.getParameter<int>("verbosity");
  ...
  for ( std::vector<double>::const_iterator mH_i = mH.begin();
    ...
    HttXsectionWithTauDecays_nlo HttXsection_woAcc(sqrtS, *mH_i, pdfName.data(), verbosity);
    ...
    HttXsectionWithTauDecays_nlo HttXsection_wAcc(sqrtS, *mH_i, pdfName.data(), verbosity);
