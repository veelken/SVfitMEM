...
namespace 
{
  ...
  {    
    ...
    double retVal = SVfitIntegrand_nlo::gSVfitIntegrand->Eval(x);
...
SVfitMEM_nlo::SVfitMEM_nlo(double sqrtS, const std::string& pdfName, int verbosity) 
  : integrand_(0),
    sqrtS_(sqrtS),
    intMode_(kVAMP),
    intAlgo_(0),
...
void SVfitMEM_nlo::setCrossSection(const TGraphErrors* graph_xSection)
{
...
void SVfitMEM_nlo::setCrossSection_and_Acc(const TGraphErrors* graph_xSection, const TGraphErrors* graph_Acc, double minAcc)
{
  graph_xSection_ = graph_xSection;
...
void
SVfitMEM_nlo::integrate(const std::vector<MeasuredTauLepton>& measuredTauLeptons, 
			double hadRecoilPx, double hadRecoilPy, double hadRecoilPz, const TMatrixD& covHadRecoil, 
			const std::string& likelihoodFileName)
{
  if ( verbosity_ >= 1 ) {
  ...
  double hadRecoilPx_rounded = roundToNdigits(hadRecoilPx);
  double hadRecoilPy_rounded = roundToNdigits(hadRecoilPy);
  double hadRecoilPz_rounded = roundToNdigits(hadRecoilPz);
  TMatrixD covHadRecoil_rounded(2,2);
  covHadRecoil_rounded[0][0] = roundToNdigits(covHadRecoil[0][0]);
  covHadRecoil_rounded[1][0] = roundToNdigits(covHadRecoil[1][0]);
  covHadRecoil_rounded[0][1] = roundToNdigits(covHadRecoil[0][1]);
  covHadRecoil_rounded[1][1] = roundToNdigits(covHadRecoil[1][1]);
  if ( verbosity_ >= 1 ) {
    std::cout << "hadRecoil: Px = " << hadRecoilPx_rounded << ", Py = " << hadRecoilPy_rounded << ", Pz = " << hadRecoilPz_rounded << std::endl;
    std::cout << "covHadRecoil:" << std::endl;
    covHadRecoil_rounded.Print();
    TMatrixDSym covHadRecoil_sym(2);
    covHadRecoil_sym(0,0) = covHadRecoil_rounded[0][0];
    covHadRecoil_sym(0,1) = covHadRecoil_rounded[0][1];
    covHadRecoil_sym(1,0) = covHadRecoil_rounded[1][0];
    covHadRecoil_sym(1,1) = covHadRecoil_rounded[1][1];
    TMatrixD EigenVectors(2,2);
    EigenVectors = TMatrixDSymEigen(covHadRecoil_sym).GetEigenVectors();
    ...
    EigenValues = TMatrixDSymEigen(covHadRecoil_sym).GetEigenValues();
...
//--- determine dimension of integration space 
  ...
  integrand_->setInputs(measuredTauLeptons_rounded, hadRecoilPx_rounded, hadRecoilPy_rounded, hadRecoilPz_rounded, covHadRecoil_rounded);
  ...
  SVfitIntegrand_nlo::gSVfitIntegrand = integrand_;
  ...
  if ( intMode_ == kMarkovChain ) {
    ...
    std::cerr << "<SVfitMEM_nlo::integrate>: Invalid Configuration Parameter 'intMode' = " << intMode_ << " --> ABORTING !!\n";
    assert(0);
  }
