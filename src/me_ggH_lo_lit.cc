...
me_ggH_lo_lit::me_ggH_lo_lit(const std::string& pdfName, bool applyNWA, bool includeHtoTauTauDecay)
  : applyNWA_(applyNWA),
...
me_ggH_lo_lit::~me_ggH_lo_lit() 
{
  delete pdf_;
}

double me_ggH_lo_lit::getMatrixElement() const
{
  ...
  if ( includeHtoTauTauDecay_ ) {
    if ( !br_isInitialized_ ) {
      std::cerr << "Error in <me_ggH_lo_lit::getMatrixElement>: Branching ratio for decay of Higgs into taus has not been initialized !!" << std::endl;
      assert(0);
    }
...
double me_ggH_lo_lit::getMatrixElement_woHtoTauTauDecay() const
{
  if ( !(mH_isInitialized_ && GammaH_isInitialized_) ) {
    std::cerr << "Error in <me_ggH_lo_lit::getMatrixElement_woHtoTauTauDecay>: Higgs mass and width have not been initialized !!" << std::endl;
    assert(0);
  }
