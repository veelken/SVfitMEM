...
namespace 
{
  ...
  {    
    ...
    double retVal = HttXsectionIntegrandWithTauDecays_lo::gHttXsectionIntegrandWithTauDecays->Eval(x);
...
HttXsectionWithTauDecays_lo::HttXsectionWithTauDecays_lo(double sqrtS, double mH, const std::string& pdfName, int mode, const std::string& madgraphFileName, int verbosity) 
  : applyMEtTF_(false),
...
{ 
  integrand_ = new HttXsectionIntegrandWithTauDecays_lo(sqrtS_, mH_, pdfName, mode, madgraphFileName, verbosity_);
  integrand_->setApplyMEtTF(applyMEtTF_);

  clock_ = new TBenchmark();
}

HttXsectionWithTauDecays_lo::~HttXsectionWithTauDecays_lo() 
{
  delete integrand_;
...
void
HttXsectionWithTauDecays_lo::integrate(int tau1Type, int tau1DecayMode, double vis1Mass, int tau2Type, int tau2DecayMode, double vis2Mass, const TMatrixD& covMET)
{
  //if ( verbosity_ >= 1 ) {
    std::cout << "<HttXsectionWithTauDecays_lo::integrate>:" << std::endl;
    clock_->Start("<HttXsectionWithTauDecays_lo::integrate>");
  //}
...
//--- determine dimension of integration space 
  ...
  HttXsectionIntegrandWithTauDecays_lo::gHttXsectionIntegrandWithTauDecays = integrand_;
  ...
  if ( intMode_ == kMarkovChain ) {
    ...
    std::cerr << "<HttXsectionWithTauDecays_lo::integrate>: Invalid Configuration Parameter 'intMode' = " << intMode_ << " --> ABORTING !!\n";
    assert(0);
  }
  ...
  //if ( verbosity_ >= 1 ) {
    clock_->Show("<HttXsectionWithTauDecays_lo::integrate>");
  //}
}
