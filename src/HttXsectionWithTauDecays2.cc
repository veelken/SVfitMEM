#include "TauAnalysis/SVfitMEM/interface/HttXsectionWithTauDecays2.h"
...
namespace 
{
  ...
  {    
    ...
    double retVal = HttXsectionIntegrandWithTauDecays2::gHttXsectionIntegrandWithTauDecays->Eval(x);
...
HttXsectionWithTauDecays2::HttXsectionWithTauDecays2(double sqrtS, double mH, const std::string& pdfName, int mode, const std::string& madgraphFileName, int verbosity) 
  : applyMEtTF_(false),
    applyAcceptanceCuts_(false),
    applyZeroPtConstraint_(false),
    max_v_r_(sqrtS),
    integrand_(0),
    sqrtS_(sqrtS),
    mH_(mH),
...
{ 
  integrand_ = new HttXsectionIntegrandWithTauDecays2(sqrtS_, mH_, pdfName, mode, madgraphFileName, verbosity_);
  integrand_->setApplyMEtTF(applyMEtTF_);

  clock_ = new TBenchmark();
}

HttXsectionWithTauDecays2::~HttXsectionWithTauDecays2() 
{
  delete integrand_;
...
void 
HttXsectionWithTauDecays2::shiftVisPt(bool value, TFile* inputFile)
{
  shiftVisPt_ = value;
  if ( shiftVisPt_ ) {
    delete lutVisPtResDM0_;
...
void
HttXsectionWithTauDecays2::integrate(int tau1Type, int tau1DecayMode, double vis1Mass, int tau2Type, int tau2DecayMode, double vis2Mass, const TMatrixD& covMET)
{
  //if ( verbosity_ >= 1 ) {
    std::cout << "<HttXsectionWithTauDecays2::integrate>:" << std::endl;
    clock_->Start("<HttXsectionWithTauDecays2::integrate>");
  //}
...
//--- determine dimension of integration space 
  ...
  numDimensions_ = 4; // vis1Px - vis2Px, vis1Py - vis2Py, vis1Pz and vis2Pz
  if ( !applyZeroPtConstraint_ ) {
    numDimensions_ += 2; // vis1Px + vis2Px, vis1Py + vis2Py
  }
  if ( applyMEtTF_ ) {
  ...
  integrand_->setApplyZeroPtConstraint(applyZeroPtConstraint_);
  ...
  HttXsectionIntegrandWithTauDecays2::gHttXsectionIntegrandWithTauDecays = integrand_;
  ...
  if ( intMode_ == kMarkovChain ) {
    ...
    std::string treeFileName = Form("SVfitIntegratorMarkovChain_HttXsectionWithTauDecays2_mH%03.0f", mH_);
    ...
    std::cerr << "<HttXsectionWithTauDecays2::integrate>: Invalid Configuration Parameter 'intMode' = " << intMode_ << " --> ABORTING !!\n";
    assert(0);
  }
  ...
  if ( applyZeroPtConstraint_ ) {
    ...
    xl_[0] = -TMath::Power(sqrtS_, 1./3.); // u_x^1/3 with u_x = (vis1Px - vis2Px)
    xu_[0] = +TMath::Power(sqrtS_, 1./3.);
    xl_[1] = -TMath::Power(sqrtS_, 1./3.); // u_y^1/3 with u_y = (vis1Py - vis2Py)
    xu_[1] = +TMath::Power(sqrtS_, 1./3.);
  } 
  ...
  int idx = 4;
  if ( !applyZeroPtConstraint_ ) {
    xl_[4] = 0.;           // v_r = sqrt((vis1Px + vis2Px)^2 + (vis1Py + vis2Py)^2)
    xu_[4] = max_v_r_;
    xl_[5] = -TMath::Pi(); // v_phi = arctan((vis1Py + vis2Py)/(vis1Px + vis2Px))
    xu_[5] = +TMath::Pi();
    idx += 2;
  }
  //-----------------------------------------------------------------------------
  if ( applyMEtTF_ ) {
    xl_[idx] = -100.;      // MET: recPx - genPx
    xu_[idx] = +100.;        
    xl_[idx + 1] = -100.;  // MET: recPy - genPy
    xu_[idx + 1] = +100.;  
  }
  xl_[idxLeg1_X] = 0.;
  xu_[idxLeg1_X] = 1.;
  ...
  if ( !applyZeroPtConstraint_ ) {
    double area = TMath::Pi()*square(max_v_r_);
    std::cout << "dividing xSection = " << xSection_ << " +/- " << xSectionErr_ << " by area = " << area << std::endl;
    xSection_ /= area;
    xSectionErr_ /= area;
  }
  ...
  //if ( verbosity_ >= 1 ) {
    clock_->Show("<HttXsectionWithTauDecays2::integrate>");
  //}
}
