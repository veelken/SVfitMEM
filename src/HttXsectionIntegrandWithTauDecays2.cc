...
const HttXsectionIntegrandWithTauDecays2* HttXsectionIntegrandWithTauDecays2::gHttXsectionIntegrandWithTauDecays = 0;
int HttXsectionIntegrandWithTauDecays2::gNumInstances = 0;

HttXsectionIntegrandWithTauDecays2::HttXsectionIntegrandWithTauDecays2(double sqrtS, double mH, const std::string& pdfName, int mode, const std::string& madgraphFileName, int verbosity) 
  : mode_(mode),
    applyMEtTF_(true),    
    mH_(mH),
    mH2_(mH*mH),
...
{
  if ( verbosity_ ) {
    std::cout << "<HttXsectionIntegrandWithTauDecays2::HttXsectionIntegrandWithTauDecays2>:" << std::endl;
  }
  ...
  if ( madgraphFileName != "" ) {
    ...
    std::cerr << "Error in <HttXsectionIntegrandWithTauDecays2>: No param.dat file for Madgraph given !!" << std::endl;
    assert(0);
  }
  if ( mode_ == kMadgraph ) {
...
HttXsectionIntegrandWithTauDecays2::~HttXsectionIntegrandWithTauDecays2()
{
  std::cout << "<HttXsectionIntegrandWithTauDecays2::~HttXsectionIntegrandWithTauDecays2>:" << std::endl;
  delete pdf_;
  ...
  outputFileName_ = Form("HttXsectionIntegrandWithTauDecays2_mH%03.0f", mH_);
...
void HttXsectionIntegrandWithTauDecays2::updateNumDimensions()
{
  numDimensions_ = -1;
...
void 
HttXsectionIntegrandWithTauDecays2::shiftVisPt(bool value, const TH1* leg1lutVisPtRes, const TH1* leg2lutVisPtRes)
{
  shiftVisPt_ = value;
  if ( shiftVisPt_ ) {
...
void 
HttXsectionIntegrandWithTauDecays2::setInputs(int tau1Type, double vis1Mass, int tau2Type, double vis2Mass, const TMatrixD& covMET)
{
  if ( verbosity_ ) {
    std::cout << "<HttXsectionIntegrandWithTauDecays2::setInputs>:" << std::endl;
  }
...
double
HttXsectionIntegrandWithTauDecays2::Eval(const double* x) const 
{
  if ( verbosity_ >= 2 ) {
    std::cout << "<HttXsectionIntegrandWithTauDecays2::Eval(const double*)>:" << std::endl;
  ...
  double v_x = 0.;
  double v_y = 0.;
  if ( !applyZeroPtConstraint_ ) {
    u_x = TMath::Power(x[0], 3.);
    u_y = TMath::Power(x[1], 3.);
    double v_r = x[4];
    double v_phi = x[5];
    v_x = v_r*TMath::Cos(v_phi);
    v_y = v_r*TMath::Sin(v_phi);
  }
  ...
  double vis1Px, vis1Py;
  if ( applyZeroPtConstraint_ ) {
    vis1Px = visPtShift1*x[0];
    vis1Py = visPtShift1*x[1];
  } else {
    vis1Px = visPtShift1*x1*0.5*(u_x + v_x);
    vis1Py = visPtShift1*x1*0.5*(u_y + v_y);
  }
  ...
  double mVis2 = x[3];
  if ( verbosity_ >= 3 ) {
    std::cout << "mVis = " << TMath::Sqrt(mVis2) << std::endl;
  }
  ...
  double vis2Px, vis2Py;
  if ( applyZeroPtConstraint_ ) {
    vis2Px = -x2*vis1Px/x1;
    vis2Py = -x2*vis1Py/x1;
  } else {
    vis2Px = -visPtShift1*x2*0.5*(u_x - v_x);
    vis2Py = -visPtShift1*x2*0.5*(u_y - v_y);
  }
...
double
HttXsectionIntegrandWithTauDecays2::compProb(const double* x, const LorentzVector& vis1P4, double x1, const LorentzVector& vis2P4, double x2, double absDgDp2z) const
{
  ...
  int idx = ( applyZeroPtConstraint_ ) ? 4 : 6; 
  double residualX = x[idx];
  double residualY = x[idx];
  ...
  if ( verbosity_ >= 3 ) {
    ...
    std::cout << "   x1 = " << (vis1En/tau1En) << std::endl;
    ...
    std::cout << "   x2 = " << (vis2En/tau2En) << std::endl;
  ...
  double mVis2 = square((vis1P4_mem + vis2P4_mem).mass());
  double q4 = square(q2_);
  jacobiFactor *= (mVis2/(q4*x1_mem));                                        // transformation from x2 to q^2
  ...
  if ( applyZeroPtConstraint_ ) {
    jacobiFactor *= square(x2);                                               // from delta-functions (tau1Px - tau2Px)*(tau1Py - tau2Py) = (vis1Px/x1 - vis2Px/x2)*(vis1Py/x1 - vis2Py/x2) 
  } else {
    jacobiFactor *= square(0.5*x1*x2);                                        // CV: transformation from vis1Px, vis2Px to u_x, v_x and from vis1Py, vis2Py to u_y, v_y 
    jacobiFactor *= 3.*square(x[0]);                                          // CV: reparametrization of u_x by u_x^3
    jacobiFactor *= 3.*square(x[1]);                                          // CV: reparametrization of u_y by u_y^3
    jacobiFactor *= x[4];                                                     // CV: transformation from v_x, v_y to v_r, v_phi
  }
  ...
  isFirstCall_ = false;
  //if ( prob < 0. || prob > 1.e+2 ) {
  //  std::cout << "--> CHECK !!" << std::endl;
  //  std::cout << "xa = " << xa << ", xb = " << xb << ": q2 = " << q2_ << std::endl;
  //  std::cout << "vis1: Pt = " << vis1P4.pt() << ", eta = " << vis1P4.eta() << ", phi = " << vis1P4.phi() << ", mass = " << vis1P4.mass() << " (Pz = " << vis1P4.pz() << ")" << std::endl;
  //  std::cout << "vis2: Pt = " << vis2P4.pt() << ", eta = " << vis2P4.eta() << ", phi = " << vis2P4.phi() << ", mass = " << vis2P4.mass() << " (Pz = " << vis2P4.pz() << ")" << std::endl;
  ...
  //  std::cout << "x1 = " << (vis1En/tau1En) << " (in MEM-frame = " << x1_mem << "), x2 = " << (vis2En/tau2En) << " (in MEM-frame = " << x2_mem << ")" << std::endl;
